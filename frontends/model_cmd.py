    ...
    for url in urls:
        for h in heads:
            try:
                ...
                err = Exception
    raise err or Exception("所有端点均无法获取模型列表")


def set_model(agent, model: str, sub: Optional[int] = None) -> str:
    """运行时改 model(仅在当前内存 session 生效, 不持久化)。"""
    s, mixin = _resolve(agent, sub)
    old = s.model
    s.model = model
    if mixin and sub is None:
        # broadcast
        for i, ss in enumerate(mixin._sessions):
            ss.model = model
        where = f"{s.name}(广播到 {len(mixin._sessions)} 子渠道)"
    else:
        where = s.name if not mixin else f"{mixin._sessions[sub].name if sub is not None else s.name}"
    return f"{where}: {old} → {model}"


# --------------------------------------------------------------------------- #
# /effort 命令 (reasoning_effort 档位切换)
# --------------------------------------------------------------------------- #
